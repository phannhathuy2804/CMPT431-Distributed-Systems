...
inline void heat_transfer_calculation(uint tid, uint size, uint start, uint end, double *time_taken, TemperatureArray* T, uint steps, CustomBarrier * barrier) {
  timer t1;
  ...
  if (tid == 0) {
    ...
    //std::cout<<"Thread: " << tid << " at step:  "<< stepcount <<  "\n";
    barrier->wait();
	  T->SwapArrays();
    ...
    //std::cout<<"Thread: " << tid << " is waiting for thread 0 "<< " at step:  "<< stepcount<<"\n";
    barrier->wait();
    barrier->wait();
	  }
  }  // end of current step
...
void heat_transfer_calculation_serial(uint size, uint number_of_threads, TemperatureArray* T, uint steps) {
  ...
  double *time_taken_of_thread = new double[number_of_threads];
  ...
  for (uint i = 0; i < number_of_threads; i++){
    threads[i] = std::thread(heat_transfer_calculation, i, size, startx[i], endx[i], &time_taken_of_thread[i], T, steps, barrier);
  }
  ...
  for (uint i = 0; i < number_of_threads; i++){
    std::cout << i << ", " << startx[i] << ", " << endx[i] << ", " << std::setprecision(TIME_PRECISION) << time_taken_of_thread[i] <<"\n";
  }
