...
inline void pageRankCalculation(Graph* g, int max_iters, uint n_threads, uintV n, PageRankType *pr_curr, std::atomic<PageRankType> *pr_next, uint start, uint end, uint tid, double *time_taken, CustomBarrier* barrier, std::mutex *mtx){
  timer t1;
  t1.start();
...
void pageRankParallel(Graph *g, int max_iters, uint n_threads) {
  ...
  std::vector<std::thread> threads (n_threads);
    ...
    for (int i = 0; i < n_threads; i++){
        threads[i] = std::thread(pageRankCalculation, g, max_iters, n_threads, n, std::ref(pr_curr), std::ref(pr_next), start[i], end[i], i, &time_taken_of_thread[i], barrier, std::ref(mtx));
    }
